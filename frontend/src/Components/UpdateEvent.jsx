...
const UpdateEvent = () => {
    ...
    const fetchApi = async () => {
        try {
            let result = await fetch('http://127.0.0.1:8000/api/getsingleevent/' + id);
    ...
    const editEvent = async () => {
        ...
        let result = await fetch("http://127.0.0.1:8000/api/updateevent/" + id + "?_method=PUT", {
            method: 'POST',
            body: formData
        });
    ...
    return (
        <>
            <Header />
            <h2 className=' text-center'> Update Event </h2> <br />
            ...
            <button className=' btn btn-primary' onClick={() => editEvent(data.id)}> Update Event  </button>
            <button className="btn btn-outline-success"> <NavLink to='/eventlist'> Check Out the Updated list </NavLink> </button>
        </>
    )
}
