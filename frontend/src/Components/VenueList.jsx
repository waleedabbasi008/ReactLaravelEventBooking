...
const VenueList = () => {
    ...
    const fetchData = async () => {
        let response = await fetch('http://127.0.0.1:8000/api/getvenue');
        if (!response.ok) {
    ...
    const deleteItems = async (id) => {
        let result = await fetch('http://127.0.0.1:8000/api/deletevenue/' + id, {
            method: 'DELETE'
        });
            ...
            <Container className=' container'>
                {
                    ...
                    data.length === 0 ? <p className=' text-center text-primary-emphasis'>  <NavLink to='/addevent' className=' nav-link nav-item'>   No venue added yet click here to add </NavLink>  </p> :
                        data.map((elem) => {
                            return (
                                <Card key={elem.id}>
                                    <div className="card">
                                        ...
                                        <div className=' card-text' >
                                            ...
                                            <button className="btn btn-outline-primary"><NavLink to='/addvenue' className=' text-decoration-none'> Add more </NavLink></button>
                                        </div>
                                    </div>
                                </Card>
                            )
